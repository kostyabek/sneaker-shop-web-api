...
namespace BaseCamp_Web_API.Api.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        {
            return Ok(_mapper.
                Map<IEnumerable<GetUserResponse>>(await _userRepository
        ...
        {
            IEnumerable<GetUserResponse> response = new List<GetUserResponse>();
            try
            {
                response = _mapper.Map<IEnumerable<GetUserResponse>>(await _userRepository.GetByIdAsync(id));
                if (!response.Any())
                {
                    return NotFound();
                }
        ...
        /// </returns>
        [ClaimRequirement(ResourceTypes.Users, ActionTypes.Update)]
        [HttpPut("{id:int}")]
        public async Task<IActionResult> UpdateGeneralInfoAsync([FromRoute] int id, [FromBody] UpdateUserRequest userRequest)
        {
        ...
        /// Status code 500 returned if database error encountered.
        /// </returns>
        [ClaimRequirement(ResourceTypes.Users, ActionTypes.Delete)]
        [HttpDelete("{id:int}")]
